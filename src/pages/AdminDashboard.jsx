...
export default function AdminDashboard() {
  ...
  const handleSalvar = async (e) => {
    ...
    // MUDANÇA TOTAL: Trocado de PUT para PATCH para resolver o erro 405 do seu servidor
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`❌ Erro ${res.status}: O servidor não aceitou o salvamento. Tente novamente.`);
      }
    } catch (err) { 
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl border-4 border-green-800 p-6 animate-in zoom-in-95">
              ...
              <div className="max-h-[400px] overflow-y-auto space-y-4">
                {(!historicoAberto.historico_pontos || historicoAberto.historico_pontos.length === 0) ? <p className="text-center text-gray-400 py-10 uppercase text-[10px] font-black tracking-widest">Vazio</p> : 
                      ...
                      <td className="p-6 text-center">
                        {pontuandoId === m._id ? (
                          ...
                          <div className="flex justify-center gap-2">
                            ...
                            <button onClick={() => abrirEdicao(m)} className="bg-amber-400 text-white p-2.5 rounded-xl active:scale-95 shadow-md">✏️</button>
                            <button onClick={() => deletarMembro(m._id)} className="bg-red-600 text-white p-2.5 rounded-xl active:scale-95 shadow-md">🗑️</button>
                          </div>
                        )}
                      </td>
                    </tr>
