  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`❌ Erro ${res.status}: Não foi possível salvar.`);
      }
    } catch (err) { 
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-white/70 backdrop-blur-md shadow-2xl transform transition-transform duration-300 border-r border-white/20 ${menuLateralAberto ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 text-center text-green-900 flex flex-col h-full">
            ...
            <div className="flex flex-col gap-4">
                <button onClick={() => { setView('cadastro'); setMenuLateralAberto(false); }} className="bg-green-600 text-white p-4 rounded-2xl font-black uppercase text-xs shadow-md active:scale-95 hover:bg-green-700">+ NOVO DESBRAVADOR</button>
                <button onClick={() => { navigate('/admin-unidades'); setMenuLateralAberto(false); }} className="bg-yellow-50 text-green-950 p-4 rounded-2xl font-black uppercase text-xs shadow-md active:scale-95 hover:bg-yellow-600">🛡️ GERENCIAR UNIDADES</button>
                <button onClick={() => { navigate('/chamada'); setMenuLateralAberto(false); }} className="bg-blue-600 text-white p-4 rounded-2xl font-black uppercase text-xs shadow-md active:scale-95 hover:bg-blue-700">📅 FREQUÊNCIA "CHAMADA"</button>
            </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl border-4 border-green-800 p-6">
              <div className="flex justify-between items-center mb-4">
                <div><h3 className="font-black uppercase italic text-sm">Histórico de Pontos</h3><p className="text-[10px] opacity-70 uppercase font-bold">{historicoAberto.nome}</p></div>
                <button onClick={() => setHistoricoAberto(null)} className="text-gray-400 font-black text-xl">✕</button>
              </div>
              <div className="max-h-[400px] overflow-y-auto space-y-4">
                {(!historicoAberto.historico_pontos || historicoAberto.historico_pontos.length === 0) ? (
                  <p className="text-center text-gray-400 py-10 uppercase text-xs">Nenhum registro encontrado</p>
                ) : (
                  historicoAberto.historico_pontos.map((h, i) => (
                    <div key={i} className="flex justify-between items-center p-3 bg-gray-50 rounded-2xl border-l-4 border-yellow-500 shadow-sm">
                      <div className="flex-1 pr-4"><p className="text-[10px] font-black uppercase text-green-900 leading-none mb-1">{h.motivo}</p><p className="text-[8px] text-gray-400 font-bold uppercase">{h.data}</p></div>
                    ...
                    <tr key={m._id} className="border-b last:border-0 hover:bg-green-50 transition-colors">
                      <td className="p-6">
                        <div className="flex items-center gap-4">
                          ...
                          />
                          <div><p className="font-black text-gray-800 uppercase leading-none mb-1 text-sm md:text-base tracking-tight">{m.nome}</p><p className="text-[10px] font-bold text-gray-400 uppercase italic tracking-wider">{m.funcao}</p></div>
                        </div>
                      </td>
                      ...
                      <td className="p-6 text-center">
                        {pontuandoId === m._id ? (
                          ...
                          <div className="flex justify-center gap-2">
                            <button onClick={() => setPontuandoId(m._id)} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-black shadow-md text-[10px] uppercase active:scale-95">⭐ Pontuar</button>
