...
export default function AdminDashboard() {
  ...
  // Avatar padrão para evitar o pisca-pisca
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("❌ Erro ao salvar. Verifique se a foto é muito pesada ou se os campos estão corretos.");
      }
    } catch (err) { 
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-white/70 backdrop-blur-md shadow-2xl transform transition-transform duration-300 border-r border-white/20 ${menuLateralAberto ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 text-center text-green-900 flex flex-col h-full">
            ...
            <div className="flex flex-col gap-4">
                <button onClick={() => { setView('cadastro'); setMenuLateralAberto(false); }} className="bg-green-600 text-white p-4 rounded-2xl font-black uppercase text-xs shadow-md active:scale-95 hover:bg-green-700 transition-colors">+ NOVO DESBRAVADOR</button>
                <button onClick={() => { navigate('/admin-unidades'); setMenuLateralAberto(false); }} className="bg-yellow-50 text-green-950 p-4 rounded-2xl font-black uppercase text-xs shadow-md active:scale-95 hover:bg-yellow-600 transition-colors">🛡️ GERENCIAR UNIDADES</button>
                <button onClick={() => { navigate('/chamada'); setMenuLateralAberto(false); }} className="bg-blue-600 text-white p-4 rounded-2xl font-black uppercase text-xs shadow-md active:scale-95 hover:bg-blue-700 transition-colors">📅 FREQUÊNCIA "CHAMADA"</button>
            </div>
            ...
            <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl border-4 border-green-800 p-6">
              ...
              <div className="max-h-[400px] overflow-y-auto space-y-4 pr-2">
                {(!historicoAberto.historico_pontos || historicoAberto.historico_pontos.length === 0) ? (
                  <p className="text-center text-gray-400 py-10 uppercase text-[10px] font-black">Nenhum registro</p>
                ) : (
                  historicoAberto.historico_pontos.map((h, i) => (
                    <div key={i} className="flex justify-between items-center p-3 bg-gray-50 rounded-2xl border-l-4 border-yellow-500 shadow-sm transition-transform hover:scale-[1.02]">
                      <div className="flex-1"><p className="text-[10px] font-black uppercase text-green-900 leading-none mb-1">{h.motivo}</p><p className="text-[8px] text-gray-400 font-bold">{h.data}</p></div>
        ...
        {view === 'tabela' ? (
                ...
                <tbody>
                  {membros.map(m => (
                    <tr key={m._id} className="border-b last:border-0 hover:bg-green-50 transition-colors">
                      ...
                      <td className="p-6 text-center"><button onClick={() => setHistoricoAberto(m)} className="bg-green-100 hover:bg-green-200 text-green-800 px-6 py-3 rounded-2xl font-black text-2xl border border-green-200 transition-all active:scale-90">{m.pontos}</button></td>
                        ...
                        {pontuandoId === m._id ? (
                          <div className="flex flex-col gap-2 bg-yellow-50 p-4 rounded-2xl border-2 border-yellow-400 min-w-[200px]">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => salvarPontos(m._id)} className="bg-green-600 text-white flex-1 py-1 rounded-lg font-black uppercase text-[10px] hover:bg-green-700">OK</button>
          ...
          <div className="bg-white p-8 md:p-10 rounded-[40px] shadow-2xl border-4 border-green-800 mb-10 animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSalvar} className="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div className="flex flex-col gap-2"><label className="text-[10px] font-black text-gray-400 ml-4 uppercase">Foto (Opcional)</label><input type="file" className="border-2 p-3 rounded-2xl bg-gray-50 font-bold text-xs" onChange={e => setArquivo(e.target.files[0])} /></div>
               <div className="md:col-span-2 flex gap-4 mt-4">
                  <button type="submit" disabled={loading} className="bg-green-700 text-white p-5 rounded-2xl font-black flex-1 shadow-xl uppercase transition-all hover:bg-green-800 active:scale-95">
