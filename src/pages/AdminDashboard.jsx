...
export default function AdminDashboard() {
  ...
  // Bonequinho fixo que não pisca e não dá erro
  ...
  // LÓGICA DE SALVAMENTO CORRIGIDA E SIMPLIFICADA
  const handleSalvar = async (e) => {
    e.preventDefault();
    setLoading(true);
    
    // Usamos FormData para enviar texto e arquivo (foto) juntos
    const data = new FormData();
    data.append('nome', nome);
    data.append('unidade', unidade);
    data.append('funcao', funcao);
    // Só adiciona a foto se um novo arquivo foi selecionado
    if (arquivo) data.append('foto', arquivo);

    const isEdicao = view === 'edicao' && membroParaEditar;
    
    // MUDANÇA CRÍTICA: Usamos PATCH para edição e POST para novo.
    // O método PATCH resolve o erro 405 do seu servidor para FormData na edição.
    ...
    try {
      const res = await fetch(url, { 
        method: method, 
        body: data // Envia o FormData completo
      });

      if (res.ok) {
        alert(isEdicao ? "✅ Cadastro atualizado com sucesso!" : "✅ Novo desbravador salvo!");
        limparFormulario();
        carregarMembros();
      } else {
        // Exibe o status do erro para facilitar o diagnóstico
        alert(`❌ Erro do Servidor (${res.status}). Não foi possível salvar.`);
      }
    } catch (err) { 
  ...
  return (
    <div className="relative min-h-screen bg-[#f1f5f2] font-sans text-gray-800 flex flex-col overflow-x-hidden">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-white/70 backdrop-blur-md shadow-2xl transform transition-transform duration-300 border-r border-white/20 ${menuLateralAberto ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 text-center text-green-900 flex flex-col h-full overflow-y-auto pr-2">
            <div className="flex justify-end"><button onClick={() => setMenuLateralAberto(false)} className="text-gray-400 text-2xl font-black hover:text-red-500 transition-colors">✕</button></div>
            <h2 className="text-xl font-black uppercase italic mb-8 mt-2 tracking-tighter">Navegação</h2>
            <div className="flex flex-col gap-4">
                <button onClick={() => { setView('cadastro'); setMenuLateralAberto(false); }} className="bg-green-600 text-white p-4 rounded-2xl font-black uppercase text-xs active:scale-95 shadow-md hover:bg-green-700 transition-colors">+ NOVO DESBRAVADOR</button>
                <button onClick={() => { navigate('/admin-unidades'); setMenuLateralAberto(false); }} className="bg-yellow-500 text-green-950 p-4 rounded-2xl font-black uppercase text-xs shadow-md transition-all active:scale-95 hover:bg-yellow-600">🛡️ GERENCIAR UNIDADES</button>
                <button onClick={() => { navigate('/chamada'); setMenuLateralAberto(false); }} className="bg-blue-600 text-white p-4 rounded-2xl font-black uppercase text-xs hover:bg-blue-700 transition-colors">📅 FREQUÊNCIA "CHAMADA"</button>
            </div>
            <div className="mt-auto pb-6"><button onClick={() => navigate('/')} className="w-full bg-red-600 text-white p-4 rounded-2xl font-black uppercase text-xs shadow-md active:scale-95 hover:bg-red-700">SAIR DO SISTEMA</button></div>
        </div>
      </div>
      ...
      <main className="flex-1 p-2 md:p-8 mt-24 max-w-7xl mx-auto w-full overflow-y-auto">
        {historicoAberto && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl border-4 border-green-800 p-6 animate-in zoom-in-95 duration-200">
              <div className="flex justify-between items-center mb-4 border-b pb-2">
                <div><h3 className="font-black uppercase italic text-sm text-green-900 leading-none">Histórico de Pontos</h3><p className="text-[10px] opacity-70 uppercase font-bold">{historicoAberto.nome}</p></div>
                <button onClick={() => setHistoricoAberto(null)} className="text-gray-400 font-black text-xl hover:text-red-500 transition-colors">✕</button>
              </div>
              <div className="max-h-[400px] overflow-y-auto space-y-4 pr-2">
                {(!historicoAberto.historico_pontos || historicoAberto.historico_pontos.length === 0) ? <p className="text-center text-gray-400 py-10 uppercase text-[10px] font-black tracking-widest">Nenhum registro</p> : 
                  historicoAberto.historico_pontos.map((h, i) => (
                    <div key={i} className="flex justify-between items-center p-3 bg-gray-50 rounded-2xl border-l-4 border-yellow-500 shadow-sm transition-transform hover:scale-[1.02]">
                      <div className="flex-1 pr-3"><p className="text-[10px] font-black uppercase text-green-900 leading-none mb-1">{h.motivo}</p><p className="text-[8px] text-gray-400 font-bold uppercase">{h.data}</p></div>
        ...
        {view === 'tabela' ? (
                  ...
                  {membros.map(m => (
                    <tr key={m._id} className="border-b last:border-0 hover:bg-green-50 transition-colors">
                      ...
                      <td className="p-6 text-center">
                        {pontuandoId === m._id ? (
                          <div className="flex flex-col gap-2 bg-yellow-50 p-4 rounded-2xl border-2 border-yellow-400 min-w-[200px] animate-in slide-in-from-top-2 duration-300">
                            <input type="number" placeholder="Qtd" className="p-2 border rounded-lg font-bold outline-none focus:border-green-600" value={inputPontos} onChange={e => setInputPontos(e.target.value)} />
                            <input type="text" placeholder="Motivo" className="p-2 border rounded-lg text-xs outline-none focus:border-green-600" value={inputMotivo} onChange={e => setInputMotivo(e.target.value)} />
                            <div className="flex gap-2">
                                <button onClick={() => salvarPontos(m._id)} className="bg-green-600 text-white flex-1 py-1 rounded-lg font-black uppercase text-[10px] hover:bg-green-700 transition-colors">OK</button>
                                <button onClick={() => setPontuandoId('')} className="bg-red-500 text-white px-2 rounded-lg font-black hover:bg-red-600 transition-colors">✕</button>
                            </div>
                          </div>
                        ) : (
                          <div className="flex justify-center gap-2">
                            ...
                            <button onClick={() => abrirEdicao(m)} className="bg-amber-400 hover:bg-amber-500 text-white p-2.5 rounded-xl active:scale-95 shadow-md transition-colors">✏️</button>
                            <button onClick={() => deletarMembro(m._id)} className="bg-red-600 hover:bg-red-700 text-white p-2.5 rounded-xl active:scale-95 shadow-md transition-colors">🗑️</button>
                          </div>
                        )}
                      </td>
                    </tr>
          ...
          <div className="bg-white p-8 md:p-10 rounded-[40px] shadow-2xl border-4 border-green-800 mb-10 animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSalvar} className="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div className="flex flex-col gap-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-4">Foto (Opcional)</label><input type="file" className="border-2 p-3 rounded-2xl bg-gray-50 text-xs font-bold" onChange={e => setArquivo(e.target.files[0])} /></div>
               <div className="md:col-span-2 flex gap-4 mt-4 border-t pt-6">
                  <button type="submit" disabled={loading} className="bg-green-700 text-white p-5 rounded-2xl font-black flex-1 shadow-xl uppercase transition-all active:scale-95 hover:bg-green-800 disabled:bg-gray-300">
