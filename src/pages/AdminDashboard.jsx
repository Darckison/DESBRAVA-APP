...
export default function AdminDashboard() {
  ...
  // Bonequinho padrão estável (Gravatar)
  ...
  const handleSalvar = async (e) => {
    ...
    // MUDANÇA CRÍTICA: Trocado de PUT para PATCH para resolver o erro 405
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`❌ Erro do Servidor (${res.status}). Tente novamente.`);
      }
    } catch (err) { 
                ...
                {(!historicoAberto.historico_pontos || historicoAberto.historico_pontos.length === 0) ? (
                  ...
                  historicoAberto.historico_pontos.map((h, i) => (
                    <div key={i} className="flex justify-between items-center p-3 bg-gray-50 rounded-2xl border-l-4 border-yellow-500 shadow-sm">
                      <div className="flex-1"><p className="text-[10px] font-black uppercase text-green-900 leading-none mb-1">{h.motivo}</p><p className="text-[8px] text-gray-400">{h.data}</p></div>
                      ...
                      <td className="p-6 text-center">
                        {pontuandoId === m._id ? (
                          ...
                          <div className="flex justify-center gap-2">
                            ...
                            <button onClick={() => abrirEdicao(m)} className="bg-amber-400 text-white p-2.5 rounded-xl active:scale-95">✏️</button>
                            <button onClick={() => deletarMembro(m._id)} className="bg-red-600 text-white p-2.5 rounded-xl active:scale-95">🗑️</button>
                          </div>
                        )}
                      </td>
                    </tr>
