...
export default function GerenciarUnidades() {
  ...
  const handleSalvar = async (e) => {
    ...
    alert("Unidade e Logo salvas no Cloudinary!");
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl mx-auto bg-gray-50 min-h-screen font-sans">
      <div className="bg-white p-8 rounded-[40px] shadow-2xl border-t-8 border-green-800 text-center mb-10">
        <h1 className="text-3xl font-black text-green-800 italic uppercase tracking-tighter">Gerenciar Unidades</h1>
      </div>
      ...
      <form onSubmit={handleSalvar} className="bg-white p-10 rounded-[40px] shadow-2xl border-4 border-green-800 mb-10">
        <div className="space-y-6">
          ...
          <input type="number" placeholder="PONTOS EXTRAS" value={pontos} onChange={e => setPontos(e.target.value)} className="w-full border-2 p-4 rounded-2xl font-black outline-none focus:border-green-600" />
          <div className="bg-gray-50 p-4 rounded-2xl border-2 border-dashed border-gray-300">
            <input type="file" onChange={e => setArquivo(e.target.files[0])} className="text-xs font-bold w-full" />
          </div>
          <div className="flex gap-4">
            ...
            <button type="button" onClick={() => navigate('/dashboard')} className="flex-1 bg-gray-300 text-gray-700 py-5 rounded-2xl font-black uppercase">VOLTAR</button>
          </div>
        </div>
      </form>
      ...
      <div className="space-y-4">
        {unidades.map((u, i) => (
          <div key={i} className="bg-white p-5 rounded-[30px] shadow-lg flex items-center justify-between border-2 border-gray-100">
            ...
            <button onClick={async () => { if(window.confirm('Excluir?')) { await fetch(`${API_URL}/unidades/${u.nome}`, {method: 'DELETE'}); carregarUnidades(); } }} className="bg-red-500 text-white p-3 rounded-2xl shadow-md">🗑️</button>
          </div>
        ))}
      </div>
    </div>
