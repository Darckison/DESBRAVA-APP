  ...
  return (
    <div className="min-h-screen bg-green-900 bg-cover bg-center bg-fixed py-6 md:py-10 px-2 md:px-4 font-sans text-white overflow-x-hidden">
      <div className="max-w-5xl mx-auto">
        ...
        {/* PÓDIO ORGANIZADO: Ordem fixa 2º - 1º - 3º e ajuste de tamanho para Mobile */}
        <div className="flex justify-center items-end gap-1 md:gap-6 mb-20 md:mb-24 w-full">
          {podio.map((m, index) => {
            ...
            return (
              <div key={m._id} className={`flex flex-col items-center relative transition-all ${
                ...
                {/* NÚMERO DA POSIÇÃO: Ajustado para não encavalar no mobile */}
                ...
                {/* CARD COM LARGURA REDUZIDA EM VW PARA CABEREM OS 3 LADO A LADO */}
                <div className={`flex flex-col items-center p-2 md:p-6 rounded-t-[30px] md:rounded-t-[45px] border-x-2 md:border-x-4 border-t-2 md:border-t-4 shadow-2xl relative ${
                  isFirst ? 'bg-yellow-500 border-yellow-300 w-[32vw] md:w-72 min-h-[300px] md:min-h-[480px]' : 
                  isSecond ? 'bg-gray-400 border-gray-300 w-[28vw] md:w-60 min-h-[250px] md:min-h-[380px]' :
                  'bg-orange-600 border-orange-400 w-[28vw] md:w-60 min-h-[250px] md:min-h-[380px]'
                }`}>
                  
                  {/* FOTO: Reduzida no mobile */}
                  <div className="relative mb-2 md:mb-4">
                    <img src={m.foto_url} className="w-14 h-14 md:w-28 md:h-28 rounded-full border-2 md:border-4 border-white object-cover shadow-xl" alt="" />
                  </div>
                  
                  {/* NOME: Ajustado para caber no card pequeno */}
                  <h2 className="font-black text-center text-[10px] md:text-xl leading-tight text-white uppercase mb-1 drop-shadow-md px-1">
                    {m.nome.split(' ')[0]}
                  </h2>
                  
                  {/* FUNÇÃO: Apenas no computador para economizar espaço no mobile */}
                  <p className="hidden md:block text-[11px] font-bold text-black/50 uppercase italic mb-6 text-center">
                    {m.funcao}
                  </p>
                  
                  {/* PATENTE: Ajustada para mobile */}
                  <div className="bg-green-950 text-yellow-400 px-1 md:px-4 py-1 md:py-2 rounded-lg md:rounded-2xl shadow-xl border md:border-2 border-green-800 mb-4 md:mb-6 w-full max-w-[95%] flex justify-center items-center">
                    <p className="text-[7px] md:text-[10px] font-black text-center uppercase leading-none">
                      {isFirst ? getPatente(m.pontos) : getPatente(m.pontos).split(' ')[1] || getPatente(m.pontos)}
                    </p>
                  </div>
                  ...
                  <div className="mt-auto bg-black/20 w-full py-2 md:py-4 rounded-2xl md:rounded-3xl flex flex-col items-center">
                    <p className="text-xl md:text-4xl font-black text-white">{m.pontos}</p>
                    <span className="text-[7px] md:text-[9px] font-black text-white/70 uppercase tracking-tighter md:tracking-widest">PONTOS</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="bg-white rounded-[40px] overflow-hidden shadow-2xl border-4 border-white">
          <div className="bg-gray-100 p-5 text-center font-black text-gray-400 uppercase text-[10px] md:text-xs tracking-[0.3em]">
            ...
            <div key={m._id} className="flex items-center justify-between p-4 md:p-6 border-b hover:bg-green-50 transition-all">
              <div className="flex items-center gap-3 md:gap-6">
                ...
                <div>
                  ...
                  <span className="bg-green-900 text-white text-[8px] md:text-[10px] px-3 py-1 rounded-full font-black uppercase">
                    {getPatente(m.pontos)}
                  </span>
                </div>
              </div>
              <div className="text-right">
                ...
                <span className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase">PONTOS</span>
              </div>
            </div>
          ))}
        </div>
