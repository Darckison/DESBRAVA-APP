...
const UserRanking = () => {
  ...
  return (
    <div className="min-h-screen bg-[#063215] py-4 md:py-10 px-1 md:px-4 font-sans text-white">
      <div className="max-w-5xl mx-auto">
        ...
        {/* PÓDIO - FORMATO IDÊNTICO AO PRINT 1000340580.jpg */}
          ...
          {podio.map((m, index) => {
            ...
            return (
              <div key={m._id} className={`flex flex-col items-center relative ${
                ...
                {/* CARD - RETO EM CIMA, CURVADO EMBAIXO (COMO NO PRINT) */}
        ...
        <div className="bg-[#0b2915] rounded-[30px] md:rounded-[50px] overflow-hidden shadow-2xl border border-white/5 mb-10 mx-2">
          <div className="bg-white/5 py-4 text-center font-black text-white/40 uppercase text-[10px] md:text-sm tracking-[0.4em]">
            CLASSIFICAÇÃO GERAL
          </div>
            ...
            {geral.map((m, index) => (
              <div key={m._id} className="flex items-center justify-between py-5 md:py-10">
                <div className="flex items-center gap-4 md:gap-14 flex-1 min-w-0">
                  <span className="font-black text-white/20 text-xl md:text-6xl w-10 md:w-24 text-center italic">
                    {index + 4}º
                  </span>
