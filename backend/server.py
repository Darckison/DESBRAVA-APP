...
# Coloquei as chaves que você já usa para o upload automático funcionar
cloudinary.config( 
...
# --- ROTA DE MEMBROS (UPLOAD AUTOMÁTICO DO PC) ---
...
async def criar_membro(
    ...
    if foto:
        # Aqui ele pega o arquivo do seu PC e manda pro Cloudinary
        res = cloudinary.uploader.upload(foto.file)
        url_foto = res["secure_url"]

    novo = await colecao_membros.insert_one({
...
async def obter_ranking():
    ...
    ranking = []
    for uni in unidades:
        membros_m = await colecao_membros.find({"unidade": {"$regex": f"^{uni['nome']}$", "$options": "i"}}).to_list(100)
        soma = sum(m.get("pontos", 0) for m in membros_m)
        ranking.append({
            "nome": uni["nome"], "logo_url": uni.get("logo_url"),
